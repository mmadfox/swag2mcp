...
swag2mcp requires Go 1.26+.

```bash
go version
# If version < 1.26, update Go:
# https://go.dev/dl/
```
...
- **Windows PowerShell 5.1:** The TUI may display input fields on a new line. Use **Windows Terminal** or **VS Code terminal** instead — they handle raw mode correctly.
