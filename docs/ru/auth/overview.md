...
swag2mcp поддерживает **10 методов аутентификации** для работы с API, требующими авторизации. Вы настраиваете это один раз в файле конфигурации — после этого каждый вызов API через `invoke` автоматически включает правильные токены и заголовки.
...
| [`openid-connect`](/auth/openid-connect) | OIDC Discovery (client credentials) | API с поддержкой OIDC |
