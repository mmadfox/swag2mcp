...
specs:
  - domain: meteo                    # 必須、1-60文字、[a-zA-Z0-9_-]
    llm_title: Open-Meteo API             # 必須、5-120文字
    llm_instruction: |                  # オプション、最大500文字
      このAPIを使用してペット、注文、ユーザーを管理します。
    base_url: https://meteo.swagger.io/v2  # 必須、有効なURL
    disable: false                      # オプション
    ...
    auth:                               # オプション
      ...
      config:
        token: $(TOKEN_AUTH)
    collections:
      - llm_title: Open-Meteo Swagger     # オプション、最大120文字
        llm_instruction: |             # オプション、最大360文字
          Open-Meteoの主要エンドポイント
        title: ""                      # オプション、specから自動入力
        location: https://raw.githubusercontent.com/mmadfox/swag2mcp/main/specs/meteo/forecast.yml  # 必須、5-250文字
        disable: false                  # オプション
