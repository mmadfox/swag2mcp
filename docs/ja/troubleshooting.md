...
swag2mcp には Go 1.26+ が必要です。

```bash
go version
# バージョンが 1.26 未満の場合、Go を更新：
# https://go.dev/dl/
```
...
- **Windows PowerShell 5.1：** TUI が入力フィールドを新しい行に表示する場合があります。**Windows Terminal** または **VS Code ターミナル** を使用してください — これらは raw モードを正しく処理します。
