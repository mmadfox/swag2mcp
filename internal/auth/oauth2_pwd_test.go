...
func TestOAuth2PasswordAuthClient_Apply_JSONFormat(t *testing.T) {
	t.Parallel()
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body["grant_type"] != "password" || body["username"] != "testuser" || body["password"] != "testpass" {
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		...
		resp := oauth2TokenResponse{AccessToken: "json-pwd-token", TokenType: "Bearer", ExpiresIn: 3600}
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(resp)
	}))
	t.Cleanup(srv.Close)
	...
	var info Info
	require.NoError(t, client.Apply(req, &info), "Apply()")
	assert.Equal(t, "Bearer json-pwd-token", req.Header.Get(headerAuthorization))
}
